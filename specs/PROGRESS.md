...
| Task                                         | Status | Owner   | Notes                                                            |
| -------------------------------------------- | ------ | ------- | ---------------------------------------------------------------- |
| Set up Storybook with @storybook/preact-vite | [x]    | Agent-A | @storybook/preact-vite@10.x, @storybook/preact@10.x added        |
| Progressive disclosure redesign              | [x]    | Agent-A | Zustand store + CollapsibleSection component                     |
| Dark mode (system preference)                | [x]    | Agent-A | Detects prefers-color-scheme, updates on change                  |
| Ideological axes visualization               | [x]    | Agent-A | Gradient bar chart with white indicator marker                   |
| Streaming response display                   | [x]    | Agent-A | stream() method added to GeminiAdapter + StreamingText component |

### Stream B: Ownership Data

| Task                                     | Status | Owner        | Notes                                         |
| ---------------------------------------- | ------ | ------------ | --------------------------------------------- |
| Curate top 20 outlets ownership JSON     | [x]    | Agent-B      | data/ownership/ownership.json with 20 outlets |
| Ownership display component              | [x]    | Agent-B      | OwnershipCard.tsx + OwnershipUnknown.tsx      |
| Wire domain → ownership lookup into flow | [x]    | Orchestrator | background.ts + App.tsx integration           |

### Stream C: Reference Library

| Task                             | Status | Owner   | Notes                                         |
| -------------------------------- | ------ | ------- | --------------------------------------------- |
| Curate 10 reference entries JSON | [x]    | Agent-C | 10 entries in data/references/references.json |
| Heuristic reference matcher      | [x]    | Agent-C | src/lib/references/matcher.ts with scoring    |
| Further Reading UI component     | [x]    | Agent-C | FurtherReading.tsx in sidepanel               |

### Stream D: In-Page Highlights

| Task                                      | Status | Owner        | Notes                                    |
| ----------------------------------------- | ------ | ------------ | ---------------------------------------- |
| Shadow DOM highlight injection (3 colors) | [x]    | Agent-D      | src/lib/highlights/injector.ts           |
| Text range matching from AnalysisResult   | [x]    | Agent-D      | findTextRanges function                  |
| Hover tooltip with analysis summary       | [x]    | Agent-D      | Tooltip with explanation                 |
| Click highlight → scroll side panel       | [x]    | Orchestrator | PostMessage handler in content.ts        |
| Toggle highlights on/off                  | [x]    | Agent-D      | src/lib/highlights/store.ts + content.ts |

### Integration

| Task                                        | Status | Owner        | Notes                                                            |
| ------------------------------------------- | ------ | ------------ | ---------------------------------------------------------------- |
| Wire ownership + references + highlights    | [x]    | Orchestrator | App.tsx with all components, background.ts with ownership lookup |
| Update prompt for highlight-mappable output | [x]    | N/A          | Already handled in existing prompt (requests exact quotes)       |
